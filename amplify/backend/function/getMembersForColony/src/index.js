...
exports.handler = async (event) => {
  ...
  const {
    ...
    rootHash,
    domainIds: nativeDomainIds = [Id.RootDomain],
    sortingMethod = SortingMethod.BY_HIGHEST_REP,
    filteringMethod = FilteringMethod.UNION,
  ...
  const getContributorsAndWatchers = async (nativeDomainId) => {
  ...
  const contributorsAndWatchersForAllDomains = await Promise.all(
    nativeDomainIds.map((nativeDomainId) =>
      getContributorsAndWatchers(nativeDomainId),
    ),
  );

  const allContributors = contributorsAndWatchersForAllDomains.map(
    ({ contributors }) => contributors,
  );
  const allWatchers = contributorsAndWatchersForAllDomains.reduce(
    (acc, { watchers }) => [...acc, ...watchers],
    [],
  );

  if (filteringMethod === FilteringMethod.INTERSECTION) {
    const contributorsIntersection = intersectionBy(
      ...allContributors,
      ({ address: contributorAddress }) => contributorAddress,
    );

    // Not necessary for watchers since you can only watch the root domain
    return {
      contributors: contributorsIntersection,
      watchers: allWatchers,
    };
  }

  const contributorsUnion = unionBy(
    ...allContributors,
    ({ address: contributorAddress }) => contributorAddress,
  );

  return {
    contributors: contributorsUnion,
    watchers: allWatchers,
  };
};
