...
const { getExchangeRate, saveExchangeRate } = require('../api/graphql/requests');
...
const ExchangeRatesFactory = (() => {
    ...
    const getTokenIdFromAddress = async ({ tokenAddress, chainId }) => {
        ...
        try {
            ...
            const data = getTokensList();
            const token = data.find(coinItem => coinItem?.platforms?.[chain] === tokenAddress);
    ...
    const getExchangeRatesForToken = async ({
        ...
        if (tokenId) {
            ...
            exchangeRatesFromDB
                .map(({ date, marketPrice }) => ({
                    date,
                    rate: marketPrice.find((marketPriceEntry) => marketPriceEntry.currency === currency)?.rate
                }))
        ...
        const shouldFetchValuesFromAPI = existingDatesInDB.length !== uniqueDates.length;
    ...
    return {
        getExchangeRates: async (tokens, currency, chainId) => {
            ...
            const { SupportedCurrencies, SupportedNetwork } = await CoinGeckoConfig.getConfig();
