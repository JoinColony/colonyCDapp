 ...
 */
const createdAtDesc = ({ createdAt: createdAtA }: TransactionRecord, { createdAt: createdAtB }: TransactionRecord) =>
  createdAtB.getTime() - createdAtA.getTime();

/*
 * Transactions selectors.
 */
export const oneTransaction = (state: RootStateRecord, id: string): TransactionRecord | undefined =>
  state.getIn([CORE_TRANSACTIONS, CORE_TRANSACTIONS_LIST, id]) as TransactionRecord;

const allTransactions = (state: RootStateRecord): TransactionsListMap =>
  state.getIn([CORE_TRANSACTIONS, CORE_TRANSACTIONS_LIST]) as TransactionsListMap;

export const transactionByHash = (state: RootStateRecord, hash: string) =>
  createSelector(allTransactions, (transactions) => transactions.find((tx) => tx.hash === hash))(state);

export const groupedTransactions = createSelector(allTransactions, (transactions) =>
  transactions
...
export const pendingTransactions = createSelector(allTransactions, (transactions) =>
  transactions.filter((tx) => tx.status === TRANSACTION_STATUSES.PENDING).sort(createdAtDesc),
);
...
export const groupedTransactionsAndMessages = createSelector(
  ...
  (transactions, messages) =>
    transactions
       ...
       */
      .sortBy(
        (group: Collection<number, TransactionRecord>) => (group.first() as TransactionRecord).createdAt,
        // Descending createdAt order (most recent groups first)
        (createdAtA, createdAtB) => Number(createdAtB) - Number(createdAtA),
      ),
);
