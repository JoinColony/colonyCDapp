...
import { ClientType, ColonyRole, Extension, getChildIndex, getPermissionProofs, Id } from '@colony/colony-js';
...
function* moveFundsMotion({
  payload: { colonyAddress, colonyName, colonyVersion, fromDomain, toDomain, amount, tokenAddress, annotationMessage },
  ...
  try {
     ...
     */
    if (!fromDomain) {
      throw new Error('Source domain not set for MoveFundsBetweenPots transaction');
    }
    if (!toDomain) {
    ...
    const votingReputationClient = yield colonyClient.getExtensionClient(Extension.VotingReputation);
    ...
    const encodedAction = colonyClient.interface.encodeFunctionData(contractMethod, [
