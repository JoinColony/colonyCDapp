...
import { TransactionRecord, Transaction, TransactionRecordProps } from '../immutable';
import { ActionTypes } from '../actionTypes';
import { ReducerType } from '../types/reducer';
import { CoreTransactions, CoreTransactionsRecord } from '../state/transactions';
...
const transactionGroup = (tx: TransactionRecord) => {
  ...
  const id = tx.group.id.reduce((resultId, entry) => `${resultId}-${getObjectFromPath(tx, entry)}`, tx.group.key);
  return {
    ...tx.group,
    id,
  };
...
const coreTransactionsReducer: ReducerType<CoreTransactionsRecord> = (state = CoreTransactions(), action) => {
  switch (action.type) {
    case ActionTypes.TRANSACTION_CREATED: {
      ...
      return state.setIn([CORE_TRANSACTIONS_LIST, id], tx.set('group', transactionGroup(tx)));
    }
    case ActionTypes.TRANSACTION_ADD_IDENTIFIER: {
      ...
      return state.setIn([CORE_TRANSACTIONS_LIST, id, 'identifier'], identifier);
    }
    case ActionTypes.TRANSACTION_ADD_PARAMS: {
      ...
      return state.updateIn([CORE_TRANSACTIONS_LIST, id, 'params'], (originalParams) => [...originalParams, ...params]);
    }
    case ActionTypes.TRANSACTION_READY: {
      const {
        meta: { id },
      } = action;
      return state.setIn([CORE_TRANSACTIONS_LIST, id, 'status'], TRANSACTION_STATUSES.READY);
    }
    case ActionTypes.TRANSACTION_PENDING: {
      const {
        meta: { id },
      } = action;
      return state.setIn([CORE_TRANSACTIONS_LIST, id, 'status'], TRANSACTION_STATUSES.PENDING);
    }
    case ActionTypes.TRANSACTION_LOAD_RELATED: {
      ...
      return state.setIn([CORE_TRANSACTIONS_LIST, id, 'loadingRelated'], loading);
    }
    ...
    case ActionTypes.TRANSACTION_RETRY: {
      ...
      return state
        ...
        .setIn([CORE_TRANSACTIONS_LIST, id, 'status'], TRANSACTION_STATUSES.READY);
    }
