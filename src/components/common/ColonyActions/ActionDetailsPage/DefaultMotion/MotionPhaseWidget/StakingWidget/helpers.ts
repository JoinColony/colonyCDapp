...
export const getStakeFromSlider = (sliderAmount: number, remainingToStake: string, userMinStake: string) => {
...
export const getRemainingStakePercentage = (stake: BigNumber, remainingToStake: string) =>
  stake.mul(100).div(remainingToStake).isZero() ? '1' : stake.mul(100).div(remainingToStake).toString();
...
export const calculateStakeLimitDecimal = (
  ...
  const stakingLimit = userMaxStake.gte(userActivatedTokens) ? userActivatedTokens : userMaxStake;
   ...
   */
  if (userHasInsufficientReputation(userActivatedTokens, userMaxStake, remainingToStake)) {
  ...
  const adjustedRemainingToStake = BigNumber.from(remainingToStake).sub(userMinStake);

  if (adjustedStakingLimit.gte(adjustedRemainingToStake)) {
    return new Decimal(1);
  }

  const stakeDecimal = new Decimal(adjustedStakingLimit.toString()).div(adjustedRemainingToStake.toString());
