...
import { actionsSort, ActionsListHeading } from './index';
import { mockActionData } from './mockData'; // Replace with real data...
...
const ColonyActions = (/* { ethDomainId }: Props */) => {
  ...
  const sortedActionsData = mockActionData.sort(actionsSort(sortOption));
  const totalItemsDisplayed = ITEMS_PER_PAGE * pageNumber;
  const paginatedActionData = sortedActionsData.slice(0, totalItemsDisplayed);
  const loadMoreItems = totalItemsDisplayed < sortedActionsData.length;
    ...
    <div className={styles.main}>
      ...
      {sortedActionsData.length ? (
        <>
          ...
          <ActionsList items={paginatedActionData} />
          {loadMoreItems && (
            <LoadMoreButton
