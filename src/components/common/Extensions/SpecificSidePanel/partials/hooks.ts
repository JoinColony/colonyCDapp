...
export const useGetInstalledByData = (
  extensionData: AnyExtensionData,
): SpecificSidePanelHookProps | undefined => {
  ...
  const formattedUserReputations = userReputation?.map(
    ({ domainId, ...rest }) => {
  ...
  const colonyReputationItems = (formattedUserReputations || [])?.map(
