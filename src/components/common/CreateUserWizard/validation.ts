...
import { EmailPermissions, GetProfileByEmailDocument, GetUserByNameDocument } from '~gql';
...
const { formatMessage } = intl({
  ...
  'error.emailRequired': 'Enter your email to continue, or uncheck options below',
  'error.emailAlreadyRegistered': 'This email is already associated with a colony username',
...
export const stepUserEmailValidationSchema: ObjectSchema<UserWizardStep1> = object({
  email: string()
    .default(step1.email)
    .email(formatMessage({ id: 'error.validEmail' }))
    .test('isEmailAlreadyRegistered', formatMessage({ id: 'error.emailAlreadyRegistered' }), isEmailAlreadyRegistered)
  ...
  emailPermissions: array()
    .defined()
    .of(string().defined().oneOf<UserEmailPermission>([EmailPermissions.IsHuman, EmailPermissions.SendNotifications]))
    .default(step1.emailPermissions),
}).defined();

export const stepUserNameValidationSchema: ObjectSchema<UserWizardStep2> = object({
  username: string()
    .default(step2.username)
    .required(formatMessage({ id: 'error.usernameRequired' }))
    .max(255)
    .test('isValidUsername', formatMessage({ id: 'error.username' }), isValidUsername)
    .test('isUsernameTaken', formatMessage({ id: 'error.usernameTaken' }), isUsernameTaken),
}).defined();
