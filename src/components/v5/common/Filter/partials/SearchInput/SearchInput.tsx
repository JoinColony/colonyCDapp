...
const SearchInput: FC<SearchInputProps> = ({
  ...
  return (
    <div className='group relative flex w-full justify-end bg-base-white text-md text-3 after:absolute after:-left-[0.1875rem] after:-top-[0.1875rem] after:block after:h-[calc(100%+0.1875rem+0.1875rem)] after:w-[calc(100%+0.1875rem+0.1875rem)] after:rounded-xl after:border-[0.1875rem] after:border-transparent after:transition-all after:duration-normal after:content-[""] focus-within:border-blue-100 hover:after:border-blue-100'>
      <span className="pointer-events-none absolute left-3 top-[50%] z-[2] flex translate-y-[-50%] text-gray-900 sm:left-[0.75rem] sm:text-gray-500">
        <MagnifyingGlass size={14} />
      </span>
      <input
        ref={ref}
        className={clsx(
          'z-[1] min-h-[2.125rem] w-full rounded-lg border border-gray-300 px-[2.125rem] py-1.5 placeholder:text-gray-400 focus:outline-none group-focus-within:border-blue-200 group-hover:border-blue-200',
      ...
      {showClearButton && (
        <button
          className={clsx(
            'absolute right-0 top-0 z-[1] flex h-[2.25rem] w-[2.375rem] items-center justify-center text-gray-900',
      ...
      {isMobile && value && (
        <button
          type="button"
          className="absolute right-0 top-0 z-[1] flex h-full items-center justify-center rounded-lg bg-blue-400 px-3.5 text-base-white transition-all duration-normal sm:text-gray-500"
