...
import { type CreateStakedExpenditureModalProps } from './types.ts';
...
const CreateStakedExpenditureModal: FC<CreateStakedExpenditureModalProps> = ({
  ...
  const values = getCreateStakedExpenditurePayload(
    colony,
    formValues as PaymentBuilderFormValues,
    networkInverseFee,
    stakeAmount,
    stakedExpenditureAddress,
  );
      ...
      <div className="flex w-full flex-grow flex-col pb-6 pr-6 [-webkit-overflow-scrolling:touch]">
        <div className="flex-grow">
          ...
          <div className="mt-6 flex justify-between gap-4 rounded-[.25rem] bg-gray-50 px-[.875rem] py-3 text-md text-gray-900">
            <p className="font-medium">{formatText(MSG.requiredStakeAmount)}</p>
            {isLoading ? (
              <SpinnerLoader appearance={{ size: 'medium' }} />
            ) : (
              <AmountField amount={stakeAmount} tokenAddress={tokenAddress} />
            )}
          </div>
          {!hasEnoughTokens && (
