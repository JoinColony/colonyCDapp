...
import { type Colony } from '@colony/sdk';
...
>(
  ...
  // TODO: Think about whether we should really give people access to the apollo client
  const apollo = useApolloClient();
  const { user } = useAppContext();

  const transactions = txs({ apollo, colony, user });
