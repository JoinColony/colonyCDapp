...
const convertRolesToArray = (rolesInDomain?: ColonyRoleFragment | null) =>
  Object.keys(rolesInDomain || {})
    ...
    .map((keyName) => (rolesInDomain?.[keyName] ? parseInt(keyName.slice(keyName.indexOf('_') + 1), 10) : undefined))
    .filter(notUndefined);
...
export const getUserRolesForDomain = (
  ...
  const userRolesInAnyDomain = colony.roles?.items.find(
    (domainRole) => domainRole?.domain?.nativeId === domainId && domainRole?.targetAddress === userAddress,
  );

  const userRolesInRootDomain = colony.roles?.items.find(
    (domainRole) => domainRole?.domain?.nativeId === Id.RootDomain && domainRole?.targetAddress === userAddress,
  );

  if (excludeInherited) {
    return userRolesInRootDomain ? convertRolesToArray(userRolesInRootDomain) : [];
  }
...
export const getAllUserRoles = (colony: ColonyFragment, userAddress: Address): ColonyRole[] => {
  if (!colony || !userAddress) return [];

  const userRolesInAnyDomain = colony.roles?.items.find((domainRole) => domainRole?.targetAddress === userAddress);
