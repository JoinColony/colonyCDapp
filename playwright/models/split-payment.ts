...
export class SplitPayment {
  ...
  async setDistribution(distribution: Distribution) {
    await this.form.getByRole('button', { name: 'Select split type' }).click();
    await this.page
  ...
  async setAmount({ amount, row }: { amount: string; row: number }) {
  ...
  async fillForm({
    ...
    if (totalAmount) {
      await this.setTotalAmount(totalAmount);
    }
    if (distribution) {
      await this.setDistribution(distribution);
    }
    ...
    if (recipients) {
      for (const [
        ...
        if (percentage) {
          await this.setPercentage({ percentage, row });
        }
  ...
  async getPercentage(row: number) {
    return this.recipientsTable
      .getByRole('row')
      .nth(row)
      .getByPlaceholder('Enter value');
  }
  ...
  async waitForPending() {
    ...
    await this.form.getByRole('button', { name: 'Pending' }).first().waitFor({
      state: 'hidden',
    });
  }
...
type ReleaseMethod = 'Payment creator' | 'Permissions';
